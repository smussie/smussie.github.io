...
function ph_init() {
	...
	ph_dlbtn.setAttribute("href", "http://download.flogisoft.com/files/misc/softwares/prompt_helper/");
	ph_dlbtnimg = document.createElement("img");
	ph_dlbtnimg.setAttribute("src", "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAA4AAAAOAQMAAAAlhr%2BSAAAAAXNSR0IArs4c6QAAAAZQTFRFAGAHOTk527ktlgAAAAF0Uk5TAEDm2GYAAAAfSURBVAjXY2BgYGBvQEf2HxjkHzDwHwCxmRlAXAYGAJloB%2BGJ8YGpAAAAAElFTkSuQmCC");
	...
	ph_input.setAttribute("value", "\\[\\e[1;32m\\]\\u@\\h\\[\\e[0;1m\\]:\\[\\e[0;36m\\]\\w\\[\\e[0m\\]\\$ ");
	...
	ph_input.addEventListener("change",
		function() {
			ph_parse_prompt(ph_input.value);
		},
		true
		);
	ph_input.addEventListener("mouseup",
		function() {
			ph_parse_prompt(ph_input.value);
		},
		true
...
function ph_parse_title(title_str) {
	...
	title_str = title_str.replace(/([^\\]|^)\$([a-z_][a-z0-9_]*|[0-9]|\?|#|@|\*)/gi, "$1\${$2}");
	//
	for (i in title_str) {
		switch (mode) {
...
function ph_parse_prompt(prompt_str) {
	...
	prompt_str = prompt_str.replace(/([^\\]|^)\$([a-z_][a-z0-9_]*|[0-9]|\?|#|@|\*)/gi, "$1\${$2}");
	//
	for (i in prompt_str) {
		switch (mode) {
			...
			case "char":
				if (prompt_str[i] == "\\") {
					...
					ph_prev.appendChild(_mk_ph_node(
								prompt_str[i],
								fg_color,
								bg_color,
								attr_bold,
			...
			case "OSC-T":
				if (prompt_str[i] == "\u0007") {
					_clear_inner(ph_titlebar);
					ph_titlebar.appendChild(ph_dlbtn);
					ph_titlebar.appendChild(document.createTextNode(
								ph_parse_title(buff) + " - Prompt Helper"))
						mode = "char";
				} else {
					buff += prompt_str[i];
				}
